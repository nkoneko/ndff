...
static struct ndpi_flow *get_ndpi_flow(u_int16_t thread_id,
    ...
    *proto = iph->protocol;
    ...
    if(ret == NULL) {
        ...
        if((newflow->ndpi_flow = (struct ndpi_flow_struct*) malloc_wrapper(size_flow_struct)) == NULL) {
        ...
        *src = (struct ndpi_id_struct*) newflow->src_id, *dst = (struct ndpi_id_struct*) newflow->dst_id;
        return newflow;
    } else {
        ...
        if(flow->lower_ip == lower_ip && flow->upper_ip == upper_ip
           && flow->lower_port == lower_port && flow->upper_port == upper_port)
            *src = (struct ndpi_id_struct*) flow->src_id, *dst = (struct ndpi_id_struct*) flow->dst_id;
        else
            *src = (struct ndpi_id_struct*) flow->dst_id, *dst = (struct ndpi_id_struct*) flow->src_id;
...
datalink_check:
    ...
    /* check ether type */
    /* Ether frameとIPヘッダの間に挿入されるデータがある場合の処理 */
    if(type == VLAN) {
